<template>
	<view class="usernickname">
		<Head :title="user.nickname" @navigateBack="navigateBack">
			<template v-slot:right>
				<view @click="saveBtn">{{$t('commonality.save')}}</view>
			</template>
		</Head>
		<view class="nickname-c">
			<textarea v-model="nickname" maxlength="16" placeholder-style="color:#979797" 
			:placeholder="user.NickNametxt" />
			<view class="nickname-c-maxsize">{{txtVal}}/16</view>
		</view>
		
		<!-- 提示是否取消 -->
		<u-modal v-model="tipshow" :show-title="false" :show-confirm-button="false">
			<view class="tipAddress">
				<view class="tipAddress-txt">{{$t('commonality.outInfo')}}</view>
...
export default {
	...
	watch:{
		nickname(val,oldVal){
			this.txtVal = this.nickname.length;
		}
	},
	methods: {
		saveBtn(){

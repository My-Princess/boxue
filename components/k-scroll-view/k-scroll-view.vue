<template>
	<view class="k-scroll-view" ref="k-scroll-view" :style="[scrollContentStyle]" @touchstart="touchstart" @touchmove="touchmove"
	 @touchend="touchend" @touchcancel="touchcancel">
		<view v-if="refreshType === 'native' && showRefresh" class="native-refresh-icon" :style="[{ top: `${moveY}px` }]"
		 :class="[{ xuanzhun: doRefreshing }]">
			<text class="my-icons-custom icon-jiantou-refresh" style="color: #4cd964;font-size: 60rpx;"></text>
		</view>
		<view class="scroll-load-refresh" v-if="refreshType === 'custom' && showRefresh">
		...
		<view class="go-to-top-icon" v-if="old.scrollTop > 20" @tap="goTop">
			<text class="my-icons-custom icon-jiantou-up" style="color: #4cd964;font-size: 40rpx;"></text>
		</view>
	...
	export default {
		...
		methods: {
			...
			touchend: function(event) {
				...
				// console.log("hk.refreshCanDo", hk.refreshCanDo)

		...
		<scroll-view class="lee-logistics-scroll" :style="{ height }" scroll-y="true" v-if="result">
			<view class="lee-logistics">
				...
				<view class="lee-logistics-list">
					<view class="lee-logistics-msg" v-for="(v, k) of result.messages" :key="k">
	...
	export default {
		...
		methods: {
			...
			search() {
				// #ifndef H5
				uni.request({
					url: `https://api.m.sm.cn/rest?method=kuaidi.getdata&q=${this.no}`,
					success: res => this.dataHandler(res.data)
				})
				// #endif
				
				// #ifdef H5
				window.LEE_LOGISTICS_CALLBACK = res => {
					delete window.LEE_LOGISTICS_CALLBACK
					this.dataHandler(res)
				}
				const el = document.createElement('script')
				el.src = `https://api.m.sm.cn/rest?method=kuaidi.getdata&q=${this.no}&callback=LEE_LOGISTICS_CALLBACK`

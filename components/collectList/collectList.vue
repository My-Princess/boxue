	...
	<view class="collectList">
		<view class="mistakes-concent-box">
			...
			<view class="mistakes-concent-box-body" v-if="isjiantou">
				<view class="mistakes-concent-box-body-item" v-for="(item,index) in collectData" :key="item.id" @click="TopicBtn(item.id,index)" >
					<view class="mistakes-concent-box-body-item-name">{{item.subject_name}}</view>
					<view class="mistakes-concent-box-body-item-pl">{{item.count}}</view>
...
export default {
	...
	computed: {},
	methods: {
		...mapMutations('study',['getcollectid','getCollectClassId','getcollectIndex','getcollectData']),
		TopicBtn(id,index) {
			console.log('id',index)
			this.getcollectid(id);
			this.getcollectData(this.collectData)
			this.getCollectClassId(this.classId);
			this.getcollectIndex(index)
			uni.navigateTo({
				url: `/pages/questionbank/collectTopic`
			});
		},
		titleBtn(){
